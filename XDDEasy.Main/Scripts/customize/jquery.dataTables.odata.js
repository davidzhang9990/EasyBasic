...
fnServerOData = function (sUrl, aoData, fnCallback, oSettings) {
    ...
    if (oSettings.oFeatures.bServerSide) {
        ...
        $.each(oSettings.aoColumns,
            function (i, value) {
                ...
                sFieldName = sFieldName.charAt(0).toUpperCase() + sFieldName.slice(1);

                if ((oParams.sSearch !== null && oParams.sSearch !== "" || columnFilter !== null && columnFilter !== "" || value.sType == "custom") && value.bSearchable) {
                    switch (value.sType) {
                        case 'string':
                        case 'html':
                        ...
                        case 'numeric':
                            var fnFormatValue =
                                (value.sType == 'numeric') ?
                                    ...
                                    function (val) {
                                        return new Date(val).Format("MM/dd/yyyy");
                                    };
                            ...
                            break;
                        case 'object':
                            if (value.mAttribute && value.mAttribute !== "") {
                                if (oParams.sSearch !== null && oParams.sSearch !== "") {
                                    asFilters.push("indexof(tolower(" + sFieldName + "/" + value.mAttribute.charAt(0).toUpperCase() + value.mAttribute.slice(1) + "), '" + oParams.sSearch.toLowerCase() + "') gt -1");
                                }

                                if (columnFilter !== null && columnFilter !== "") {
                                    asColumnFilters.push("indexof(tolower(" + sFieldName + "/" + value.mAttribute.charAt(0).toUpperCase() + value.mAttribute.slice(1) + "), '" + oParams.sSearch.toLowerCase() + "') gt -1");
                                }
                            }
                            break;
                        case 'custom':
                            if (value.mFilter && value.mFilter.length > 0) {
                                var searchValue = "", skipValue = "";
                                if (value.mFilter.length === 1) {
                                    if (value.mSearchOptions.bOutTable) {
                                        if (typeof value.mSearchOptions.skipValue !== "undefined") {
                                            skipValue = value.mSearchOptions.skipValue.toLowerCase();
                                        }
                                        if (value.mSearchOptions.bDDL) {
                                            if (value.mSearchOptions.bSearchValue) {
                                                searchValue = $("#" + value.mSearchOptions.sSearchFeildId).val();
                                            } else {
                                                searchValue = $("#" + value.mSearchOptions.sSearchFeildId).find("option:selected").text();
                                            }
                                            if (searchValue !== "" && searchValue.toLowerCase() !== skipValue) {
                                                andFilter.push(value.mFilter[0].replace('{}', searchValue));
                                                if (columnFilter !== null && columnFilter !== "") {
                                                    asColumnFilters.push(value.mFilter[0].replace('{}', searchValue));
                                                }
                                            }
                                        } else {
                                            searchValue = $("#" + value.mSearchOptions.sSearchFeildId).val();
                                        ...
                                        if (oParams.sSearch !== null && oParams.sSearch !== "") {
                                            asFilters.push(value.mFilter[0].replace('{}', oParams.sSearch.toLowerCase()));
                                        }
                                        if (columnFilter !== null && columnFilter !== "") {
                                            asColumnFilters.push(value.mFilter[0].replace('{}', oParams.sSearch.toLowerCase()));
                                        }
                                    }
                                    ...
                                    if (value.mSearchOptions.bOutTable) {
                                        ...
                                        if (value.mSearchOptions.bDDL) {
                                            ...
                                            if (searchValue !== "" && searchValue.toLowerCase() !== skipValue) {
                                                if (value.mCondition == "=") {
                                                    if (searchValue == value.mConditionValue) {
                                                        andFilter.push(value.mFilter[1].replace('{}', searchValue));
                                                        if (columnFilter !== null && columnFilter !== "") {
                                                            asColumnFilters.push(value.mFilter[1].replace('{}', searchValue));
                                                        }
                                                    } else {
                                                        andFilter.push(value.mFilter[0].replace('{}', searchValue));
                                                        if (columnFilter !== null && columnFilter !== "") {
        ...
        if (andFilter.length > 0) {
            if (data.$filter !== undefined) {
                data.$filter = " ( " + data.$filter + " ) and ( " + andFilter.join(" and ") + " ) ";
            } else {
                data.$filter = andFilter.join(" and ");
            }
        }
        ...
        for (var i = 0; i < oParams.iSortingCols; i++) {
            var orderColumn = oParams["mDataProp_" + oParams["iSortCol_" + i]];
            var columnObj = oSettings.aoColumns[oParams["iSortCol_" + i]];
            switch (columnObj.sType) {
                case "object":
                    if (columnObj.mAttribute && columnObj.mAttribute != "") {
                        orderColumn = orderColumn.charAt(0).toUpperCase() + orderColumn.slice(1) + "/" + columnObj.mAttribute.charAt(0).toUpperCase() + columnObj.mAttribute.slice(1);
                        asOrderBy.push(orderColumn + " " + (oParams["sSortDir_" + i] || ""));
                    }
                    break;
                case "custom":
                    if (typeof columnObj.mOrder !== "undefined") {
                        asOrderBy.push(columnObj.mOrder + " " + (oParams["sSortDir_" + i] || ""));
                    }
                    break;
                default:
                    orderColumn = orderColumn.charAt(0).toUpperCase() + orderColumn.slice(1);
                    asOrderBy.push(orderColumn + " " + (oParams["sSortDir_" + i] || ""));
                    break;
            }
        }
    ...
    $.ajax(jQuery.extend({}, oSettings.oInit.ajax, {
        ...
        "success": function (data) {
            ...
            oDataSource.aaData = data.value || (data.d && data.d.results) || data.d || data.items;

; (function ($, window, document) {
    ...
    win.on('normalized-resize orientationchange', function () {
        ...
        $.modal.all.each(function (i) {
            ...
            if (data) {
                ...
                //Compatible iframe video full screen
                var iframe = modal.find('iframe');
                if (iframe) {
                    var frameWin = iframe[0].contentWindow ? iframe[0].contentWindow : iframe[0].contentDocument.defaultView;
                    if (frameWin.videoState) {
                        data.setPosition(frameWin.iframeModalLeft, frameWin.iframeModalTop);
                    }
                } else {
    ...
    $.modal = function (options) {
        /*  if (EqlResource && EqlResource.UI_Action_Close)
              options.actionCloseCaption = EqlResource.UI_Action_Close;*/
			    ...
			    modal.stop(true).animate({
			        ...
			        if (blocker) {
			            ...
			            if (root.children('.modal-blocker').length === 0) {
			                root.removeClass('with-blocker').hide();
        ...
        if (settings.blocker) {
            ...
            blocker = $('<div class="modal-blocker"></div>').appendTo(root.addClass('with-blocker').show());
        ...
        $.each(settings.actions, function (name, config) {
            ...
            // Button zone
            if (!actionsBlock) {
                actionsBlock = $('<ul class="modal-actions"></ul>').prependTo(barBlock || modal)
								.data('tooltip-options', settings.actionsTooltips);
            }
            var caption = name;
            if ((settings.isShowTopBtns || name === 'Close') && settings.actionCloseCaption)
                caption = settings.actionCloseCaption;
            if (settings.isShowTopBtns) {
                if (settings.isShowMaximize) {
                    var initWidth = settings.width, initHeight = settings.height;
                    if (!(initWidth === $(window).width() && initHeight === $(window).height())) {
                        $('<li class="expand"><a href="#" class="fa fa-plus-square-o" style="font-weight: 600;" title="' + settings.maximizeCaption + '"></a></li>').appendTo(actionsBlock).children('a').click(function (event) {
                            var link = $(this);
                            if (link.hasClass('fa-plus-square-o')) {
                                modal.data('modal').setContentSize($(window).width(), $(window).height());
                                modal.centerModal();
                                link.removeClass('fa-plus-square-o').addClass('fa-minus-square-o');
                            } else {
                                modal.data('modal').setContentSize(initWidth, initHeight);
                                modal.centerModal();
                                link.removeClass('fa-minus-square-o').addClass('fa-plus-square-o');
                            }

                            //auto change iframe body height when max the pop window
                            var ifrModal = modal.data('modal').contentBlockIframe;
                            if (ifrModal != null && ifrModal.length > 0) {
                                var div = ifrModal[0].contentWindow.document.body;
                                div.style.height = ifrModal[0].height - 20 + "px";
                ...
                // Insert
                $('<li' + (config.color ? ' class="' + config.color + '-hover"' : '') + '><a href="#" title="' + caption + '" class="fa fa-times"></a></li>').appendTo(actionsBlock).children('a').click(function (event) {
                    event.preventDefault();
        ...
        $.each(settings.buttons, function (name, config) {
            ...
            switch (name) {
                case "Close":
                    if (EqlResource && EqlResource.UI_Action_Close)
                        name = EqlResource.UI_Action_Close;
                    break;
                case "OK":
                    if (EqlResource && EqlResource.UI_Action_OK)
                        name = EqlResource.UI_Action_OK;
                    break;
                case "Cancel":
                    if (EqlResource && EqlResource.UI_Action_Cancel)
                        name = EqlResource.UI_Action_Cancel;
                    break;
            }


            // Insert
            $('<button type="button" class="button' + (config.classes ? ' ' + config.classes : '') + spacingClass + '">' + (config.caption ? config.caption : name) + '</button>').appendTo(buttonsBlock).click(function (event) {
                config.click.call(this, $(this).closest('.modal'), event);
            });
        });
	 ...
	 */
    $.modal.defaults = {
        /*custom callback when click the close icon*/
        cancelCallback: null,
        /**
        ...
        //whether show the maximize button in the top right cornor
        isShowMaximize: false,
        //Maximize button title
        maximizeCaption: "",
        //wethere show the buttons in the top right cornor
        isShowTopBtns: true,
